"""RDNA2 kernel dispatch — routes ATOM layer ops to Wave32 HIP kernels.
...
Usage in ATOM layers:
    from atom.model_ops.rdna2.dispatch import rdna2_ops
...
class RDNA2Ops:
    ...
    def probe(self) -> bool:
        ...
        # Check ATOM_RDNA2_KERNELS env var
        force = envs.ATOM_RDNA2_KERNELS
        if force in ("0", "false"):
            logger.info("RDNA2 kernels: disabled by ATOM_RDNA2_KERNELS=0")
            return False
        if force in ("1", "true"):
            self.available = True
            logger.info("RDNA2 kernels: force-enabled by ATOM_RDNA2_KERNELS=1")
            return True

        # Auto-detect via HSA_OVERRIDE_GFX_VERSION
    ...
    def _check_kernel(self, name: str) -> str:
        ...
        try:
            mod = __import__(
                f"atom.model_ops.rdna2.{name}", fromlist=[name]
            )
...
# Singleton instance — import this in ATOM layers
rdna2_ops = RDNA2Ops()
