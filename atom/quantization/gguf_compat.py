"""PRISM Q1_0 / Q1_0_G128 GGUF compatibility shim.

Registers the 1-bit quantization types (type IDs 42, 43) with the
``gguf`` Python library so GGUF files containing PRISM-quantized
weights can be loaded and CPU-dequantized.

Call ``ensure_prism_gguf_compat()`` once at startup before any GGUF
file is opened.

This module is pure Python/NumPy with zero ATOM or framework deps.
"""
...
def ensure_prism_gguf_compat() -> None:
    """Register PRISM Q1_0/Q1_0_G128 types with the gguf library.
    ...
    if getattr(gguf, "_atom_prism_gguf_compat", False):
        return
    ...
    gguf._atom_prism_gguf_compat = True
    gguf._atom_prism_gguf_original_build_tensors = original_build_tensors
