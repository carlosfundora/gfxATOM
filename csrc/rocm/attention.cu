...
__device__ __forceinline__ floatx4 to_float_fp8x4(const _B8x4& inp) {
  ...
  float4 f32x4 = atom::fp8::vec_conversion<float4, uint32_t>(
...
template <typename scalar_t, typename cache_t,
          atom::Fp8KVCacheDataType KV_DTYPE, typename OUTT, int BLOCK_SIZE,
...
__launch_bounds__(NUM_THREADS, 5) void paged_attention_ll4mi_QKV_mfma16_kernel(
  ...
  if ((local_qhead_idx < GQA_RATIO) && (qhead_element < HEAD_SIZE)) {
    ...
    if constexpr (KV_DTYPE == atom::Fp8KVCacheDataType::kAuto) {
      const int offset1 =
          lane16id /
        ...
        for (int qkratio = 0; qkratio < QK_SIZE_RATIO; qkratio++) {
          if constexpr (MFMA_TYPE == MFMAType::F16) {
            ...
            for (int n = 0; n < 2; n++) {
              ...
              Qtmp8x8.b16x4[n * 2] =
                  atom::fp8::scaled_vec_conversion<uint16_t, float2>(
...
__launch_bounds__(NUM_THREADS) void paged_attention_ll4mi_QKV_mfma4_kernel(
  ...
  #pragma unroll
    ...
    else {  // if constexpr (KV_DTYPE != atom::Fp8KVCacheDataType::kAuto)
...
__launch_bounds__(NUM_THREADS) void paged_attention_ll4mi_reduce_kernel(
  ...
  if constexpr (std::is_same<OUTT, bit8_t>::value) {
    out_ptr[threadIdx.x] =
        __hip_cvt_float_to_fp8(acc, atom::fp8::fp8_type::__default_saturation,
                               atom::fp8::fp8_type::__default_interpret);
  } else {
...
template <typename T, typename KVT, atom::Fp8KVCacheDataType KV_DTYPE,
  ...
  if (kv_cache_dtype == "auto") {
    if (query.dtype() == at::ScalarType::Half) {
      CALL_CUSTOM_LAUNCHER_BLK_HEAD(
          _Float16, _Float16, atom::Fp8KVCacheDataType::kAuto, MFMAType::F16);
    } else if (query.dtype() == at::ScalarType::BFloat16) {
      CALL_CUSTOM_LAUNCHER_BLK_HEAD(__hip_bfloat16, __hip_bfloat16,
                                    atom::Fp8KVCacheDataType::kAuto,
                                    MFMAType::F16);
    } else {
    ...
    if (query.dtype() == at::ScalarType::Half) {
      if (mfma_type == "fp8") {
        CALL_CUSTOM_LAUNCHER_BLK_HEAD(_Float16, uint8_t,
                                      atom::Fp8KVCacheDataType::kFp8E4M3,
                                      MFMAType::Fp8);
      } else {
        CALL_CUSTOM_LAUNCHER_BLK_HEAD(_Float16, uint8_t,
                                      atom::Fp8KVCacheDataType::kFp8E4M3,
